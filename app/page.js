...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch Live App Data from GitHub - Using specific commit/branch or cache-busting to bypass build
  ...
  if (!data) return (
    <div className="min-h-screen bg-[#020203] flex flex-col items-center justify-center space-y-6">
      ...
      <div className="text-blue-500 font-bold tracking-widest text-[10px] uppercase animate-pulse italic">Syncing neural_v1.0…</div>
    </div>
  );
      ...
      <div className="max-w-6xl mx-auto space-y-8 relative z-10">
        <header className="glass rounded-[2.5rem] p-8 md:p-10 bg-white/[0.01] border-white/[0.08] backdrop-blur-2xl flex flex-col md:flex-row justify-between items-start md:items-end gap-6 shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-8 md:gap-12">
            ...
            <div className="text-right">
              <div className="text-[10px] text-zinc-500 font-extrabold uppercase tracking-widest mb-1 opacity-60">Yield_24H</div>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="text-[10px] font-black text-blue-400 tracking-[0.2em] uppercase italic border-b border-blue-500/20 pb-1.5 text-left">Internal State</h2>
                            </div>
                            <div className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">v1.1.2_Stable</div>
                        </div>
                        <div className="text-left">
                            <p className="text-xl text-white font-bold leading-snug italic tracking-tight m-0 opacity-90">
                                {data.stats.find(s => s.consensus.label.includes('BUY')) 
                                    ? `“Neural trace detected for ${data.stats.find(s => s.consensus.label.includes('BUY')).id}. Probability of breakout confirmed.”`
                                    : "“Market scanning active. Monitoring horizontal liquidity compression. No immediate trade priority.”"
                                }
                            </p>
                ...
                <div className="glass rounded-[2.5rem] p-8 border-emerald-500/20 bg-emerald-500/[0.02]">
                    <div className="flex items-center justify-between mb-6 border-b border-white/5 pb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-[10px] font-black text-emerald-400 tracking-[0.2em] uppercase italic text-left">Paper holdings</h2>
                        </div>
                    ...
                    <table className="w-full text-left border-separate border-spacing-y-3">
                        <thead className="text-[9px] font-black uppercase tracking-widest text-zinc-600">
                           <tr>
                               ...
                               <th className="px-6 py-2 text-right">Quote</th>
                               <th className="px-6 py-2 text-right">Vel (24h)</th>
                               <th className="px-6 py-2 text-right">Council Consensus</th>
                           </tr>
                        </thead>
                        <tbody>
                            {data.stats.map((s) => (
                                <tr key={s.id} onClick={() => setSelectedToken(s)} className="group cursor-pointer">
                                    <td className="px-6 py-6 bg-white/[0.03] border-y border-l border-white/[0.05] rounded-l-2xl flex items-center gap-3 group-hover:bg-white/[0.05] transition-colors">
                                        <div className={`p-1.5 rounded-lg border border-white/5 ${s.isFC ? 'bg-blue-600/20 text-blue-400' : 'bg-white/5 text-zinc-700'}`}>
                                            <Coins className="w-4 h-4" />
                                        </div>
                                        <div className="text-left">
                                            <span className={`font-black text-lg block tracking-tighter uppercase transition-colors ${s.isFC ? 'text-blue-400' : 'text-zinc-100'}`}>{s.id}</span>
                                            <span className="text-[8px] font-bold text-zinc-600 uppercase tracking-widest uppercase">{s.chain || 'Network'}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-6 bg-white/[0.03] border-y border-white/[0.05] group-hover:bg-white/[0.05] text-right font-mono font-bold text-zinc-300 text-sm transition-colors tabular-nums">
                                        ${s.price > 1 ? s.price.toLocaleString(undefined, {minimumFractionDigits:2}) : s.price.toFixed(6)}
                                    </td>
                                    <td className={`px-6 py-6 bg-white/[0.03] border-y border-white/[0.05] group-hover:bg-white/[0.05] text-right font-black italic text-sm transition-colors ${s.change > 0 ? 'text-emerald-400' : 'text-rose-500'}`}>
                                        {s.change > 0 ? '↑' : '↓'} {Math.abs(s.change)}%
                                    </td>
                                    <td className={`px-6 py-6 bg-white/[0.03] border-y border-r border-white/[0.05] rounded-r-2xl group-hover:bg-white/[0.05] text-right transition-colors`}>
                                        ...
                                        <div className="flex justify-end gap-1 mt-1">
                                            {s.consensus.council.map((c, idx) => (
                                                <div key={idx} className={`w-1 h-1 rounded-full ${c.vote === 'BUY' ? 'bg-emerald-500' : (c.vote === 'SELL' || c.vote === 'REJECT' ? 'bg-rose-500' : 'bg-zinc-700')}`} />
                                            ))}
                                        </div>
                                    </td>
                                </tr>
                                        ...
                                        return (
                                            <tr key={idx} className="hover:bg-white/[0.02] transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-left">
                                                    <span className={`text-[9px] font-black px-2 py-0.5 rounded-md ${t.side === 'BUY' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                                                        {t.side || 'N/A'}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="px-6 py-4 font-mono text-[11px] text-right text-zinc-400">
                                                    ${parseFloat(t.price) < 1 ? parseFloat(t.price).toFixed(6) : parseFloat(t.price).toLocaleString()}
                                                </td>
                                                <td className="px-6 py-4 font-mono text-[11px] text-right">
                                                    {pnl ? <span className={pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}>{pnl >= 0 ? '+' : ''}{pnl}%</span> : <span className="text-zinc-700">--</span>}
                                                </td>
                                            </tr>
                                        );
                                    })}
                ...
                <div className="bg-[#18181b] border border-white/10 rounded-[3rem] w-full max-w-6xl shadow-2xl relative flex flex-col max-h-[95vh] overflow-hidden">
                    <div className="p-8 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                        <div className="flex items-center gap-4 text-left">
                            ...
                            <div>
                                ...
                                <div className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-2 border border-white/10 px-2 py-1 rounded inline-block">Neural_Trace_Analysis</div>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 ring-0 focus:ring-0 outline-none">
                        <div className="lg:col-span-8 space-y-6 text-left">
                            <div id="tradingview_chart" className="bg-black/40 border border-white/5 rounded-[2rem] h-[500px] w-full overflow-hidden shadow-inner flex flex-col items-center justify-center text-zinc-700 font-black uppercase tracking-[0.4em] text-[10px]">
                        ...
                        <div className="lg:col-span-4 bg-white/[0.03] rounded-[2.5rem] p-8 border border-white/10 flex flex-col justify-between h-full text-left">
                            <div className="space-y-8">
                                <div>
                                    <div className="text-[9px] font-black text-zinc-500 uppercase tracking-widest italic mb-4">Council_Verdict</div>
                                    <div className={`text-5xl font-black italic tracking-tighter uppercase leading-none ${selectedToken.consensus.color}`}>
                                        {selectedToken.consensus.label}
                                    </div>
                                    <div className="text-[11px] text-zinc-500 italic mt-4 font-medium leading-relaxed uppercase border-l-2 border-blue-500/30 pl-4">{selectedToken.consensus.desc}</div>
                                </div>
                                    ...
                                    {selectedToken.consensus.council.map((c, i) => (
                                        <div key={i} className="p-5 rounded-3xl border border-white/5 bg-black/40 hover:bg-black/60 transition-all">
                                            <div className="flex justify-between items-center mb-2">
                                                <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">{c.agent}</span>
                                                <span className={`text-[10px] font-black px-2 py-0.5 rounded border ${c.vote === 'BUY' ? 'text-emerald-400 border-emerald-500/20' : (c.vote === 'SELL' || c.vote === 'REJECT' ? 'text-rose-400 border-rose-500/20' : 'text-zinc-500 border-zinc-500/20')}`}>
                                                    {c.vote}
                                                </span>
                                            </div>
        ...
        <footer className="pt-20 pb-12 text-center opacity-30">
            <div className="text-[9px] font-black tracking-[0.4em] text-zinc-700 uppercase italic mb-2">TrashPanda Neural Ops • v1.2.0_PROFESSIONAL</div>
            <p className="text-[8px] text-zinc-800 font-bold uppercase tracking-widest">Encrypted Data Stream • Sovereign Liquidity Engine • Zero-Trust Consensus</p>
        </footer>
