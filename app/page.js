  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setData({ 
          total: wallet.balance_usd + holdings.reduce((acc, h) => acc + h.value, 0), 
          pnl: (((wallet.balance_usd + holdings.reduce((acc, h) => acc + h.value, 0) - 5000) / 5000) * 100).toFixed(2), 
        ...
        <header className="glass rounded-[2.5rem] p-8 md:p-10 bg-white/[0.01] border-white/[0.08] flex flex-col md:flex-row justify-between items-start md:items-end gap-6 shadow-2xl">
          <div className="space-y-4">
            <div className="flex items-center gap-4">
               <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg"><Target className="w-7 h-7 text-white" /></div>
               <h1 className="text-4xl font-black italic tracking-tighter uppercase leading-none">BULLSEYE 1.3</h1>
            </div>
            <div className="flex items-center gap-3"><div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" /><span className="text-[9px] text-zinc-500 font-extrabold tracking-[0.4em] uppercase">Production_Active</span></div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 text-left">
            <div className="lg:col-span-4 space-y-6">
                <div className="glass rounded-[2.5rem] p-8 neo-gradient border-blue-500/20 flex flex-col justify-between min-h-[300px]">
                    ...
                    <p className="text-xl text-white font-bold leading-snug italic tracking-tight m-0 opacity-90">"Monitoring horizontal liquidity compression. Evaluating neural traces across active surveillance pool."</p></div>
                </div>
                <div className="glass rounded-[2.5rem] p-8 border-emerald-500/20 bg-emerald-500/[0.02]">
                    ...
                    <div className="space-y-3">{data.holdings.map(h => (
            ...
            <div className="lg:col-span-8 glass rounded-[3rem] p-8 space-y-6 bg-white/[0.01]">
                ...
                <div className="overflow-x-auto"><table className="w-full text-left border-separate border-spacing-0">
                    ...
                    <tbody className="divide-y divide-white/5">{[...data.history].reverse().map((t, idx) => {
                        ...
                        const lp = s ? s.price : 0;
                        const ep = parseFloat(t.price) || 1;
                        const q = t.qty || (parseFloat(t.amount_usd) / ep);
                        const pct = (((lp - ep) / ep) * 100).toFixed(1);
                        const usd = (q * (lp - ep)).toFixed(2);
                        return (
                            <tr key={idx} className="hover:bg-white/[0.02]">
                                <td className="px-4 py-4 font-mono text-[9px] text-zinc-500">{t.timestamp ? t.timestamp.split(' ')[1] : 'N/A'}</td>
                                ...
                                <td className="px-4 py-4 font-mono text-right text-zinc-500 text-[10px]">${ep < 1 ? ep.toFixed(4) : ep.toLocaleString()}</td>
                                ...
                                <td className="px-4 py-4 font-mono text-right">
                                    {t.side === 'BUY' ? (<div className="flex flex-col items-end leading-none"><span className={parseFloat(pct) >= 0 ? 'text-emerald-400' : 'text-rose-500'}>{parseFloat(pct) >= 0 ? '+' : ''}{pct}%</span><span className="text-[8px] text-zinc-600 font-bold mt-1">${usd}</span></div>) : <span className="text-[9px] text-zinc-700">Settled</span>}
                                </td>
                            </tr>
                        );
                    })}</tbody>
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-xl bg-black/60 overflow-y-auto">
                <div className="bg-[#18181b] border border-white/10 rounded-[3rem] w-full max-w-6xl shadow-2xl relative flex flex-col max-h-[95vh] overflow-hidden text-left">
                    <div className="p-8 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                        <div className="flex items-center gap-4">
                            <div className="p-3 bg-blue-600 rounded-2xl"><Coins className="w-8 h-8 text-white" /></div>
                            <div><h2 className="text-3xl font-black italic tracking-tighter uppercase leading-none">{selectedToken.id} / USD</h2><div className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-2 border border-white/10 px-2 py-1 rounded inline-block">Neural_Intelligence_Trace</div></div>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 scrollbar-hide">
                        <div className="lg:col-span-8 space-y-6">
                            <div id="tradingview_chart" className="bg-black/40 border border-white/5 rounded-[2rem] h-[500px] w-full flex items-center justify-center font-black uppercase text-[10px] tracking-widest">LOADING_FEEDS...</div>
                        ...
                        <div className="lg:col-span-4 bg-white/[0.03] rounded-[2.5rem] p-8 border border-white/10 flex flex-col justify-between h-full space-y-8">
                            ...
                            <div className="mt-8 pt-6 border-t border-white/5 space-y-4 text-left"><div className="text-[9px] font-black text-zinc-600 uppercase italic">Strategy_Hub</div>
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div className="p-3 bg-white/[0.02] border border-white/5 rounded-2xl text-left"><div className="text-[8px] text-zinc-600 font-black uppercase mb-1">Entry</div><div className="text-[11px] text-zinc-300 font-mono">{selectedToken.consensus.label.includes('STRONG') ? '25%' : '15%'}</div></div>
                                </div>
                                <div className="p-4 bg-emerald-500/[0.03] border border-emerald-500/10 rounded-2xl"><div className="flex items-center gap-2 mb-1"><ShieldCheck className="w-3 h-3 text-emerald-500" /><span className="text-[9px] text-emerald-500 font-black uppercase">Safety_v1.3</span></div><p className="text-[10px] text-zinc-500 italic mt-1">Protecting cash floor of $500. All trades require 2/3 consensus.</p></div>
                            </div>
                            <a href={`https://dexscreener.com/${selectedToken.chain || 'base'}/${selectedToken.addr || ''}`} target="_blank" className="w-full py-5 bg-blue-600 rounded-[2rem] text-center font-black italic text-sm tracking-tighter text-white no-underline mt-10 block uppercase">Explore Raw Pool Feed</a>
                        </div>
                    </div>
                </div>
            </div>
