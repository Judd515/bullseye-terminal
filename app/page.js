...
export default function Dashboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        const getCouncilData = (token) => {
            ...
            if (liq < 30000) council.push({ agent: 'The Bear', vote: 'REJECT', logic: `Liquidity too thin.` });
            else if (change > 20) council.push({ agent: 'The Bear', vote: 'REJECT', logic: `Overextended.` });
            else council.push({ agent: 'The Bear', vote: 'NEUTRAL', logic: 'Risk within range.' });

            if (change > 5 && vol > (liq * 0.15)) council.push({ agent: 'The Mooner', vote: 'BUY', logic: `Momentum breakout!` });
            else if (change > 8) council.push({ agent: 'The Mooner', vote: 'BUY', logic: `Velocity detected.` });
            else council.push({ agent: 'The Mooner', vote: 'NEUTRAL', logic: 'Volume suboptimal.' });

            if (change > 3 && vol > 10000) council.push({ agent: 'The Quant', vote: 'BUY', logic: `Positive drift.` });
            else if (change < -5) council.push({ agent: 'The Quant', vote: 'SELL', logic: 'Distribution detected.' });
            else council.push({ agent: 'The Quant', vote: 'HOLD', logic: 'Standard deviation.' });
        ...
        const getConsensus = (token) => {
            ...
            let desc = "Consolidation detected.";
        ...
        setData({ 
          total: totalEquity, 
          pnl: (((totalEquity - 5000) / 5000) * 100).toFixed(2), 
  ...
  useEffect(() => {
    if (selectedToken) {
      ...
      if (widgetContainer) {
          ...
          script.async = true;
          const symbolMap = {'BANKR':'DEXSCREENER:0x22af33fe49fd1fa80c7149773dde5890d3c76f3b','CLANKER':'DEXSCREENER:0x1bc0c42215582d5a085795f4badbac3ff36d1bcb','DEGEN':'COINBASE:DEGENUSD','BTC':'BINANCE:BTCUSDT','ETH':'BINANCE:ETHUSDT'};
  ...
  if (!data) return (
    <div className="min-h-screen bg-[#020203] flex flex-col items-center justify-center space-y-6">
      ...
      <div className="text-blue-500 font-bold tracking-widest text-[10px] uppercase animate-pulse italic">Syncing Neural_Ops...</div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#020203] text-[#f4f4f5] font-sans selection:bg-blue-500/30 overflow-x-hidden relative p-4 md:p-12">
      ...
      <div className="max-w-6xl mx-auto space-y-8 relative z-10">
                ...
                <div className="glass rounded-[2.5rem] p-8 neo-gradient border-blue-500/20 flex flex-col justify-between group overflow-hidden relative min-h-[300px]">
                    <div className="space-y-6 text-left">
                        ...
                        <p className="text-xl text-white font-bold leading-snug italic tracking-tight m-0 opacity-90">
                           "Monitoring horizontal liquidity compression. Evaluating neural traces across active surveillance pool."
                        </p>
                    </div>
                </div>
                ...
                <div className="overflow-x-auto min-w-0">
                    ...
                    <div className="overflow-x-auto rounded-3xl border border-white/5 bg-white/[0.01]">
                        <table className="w-full text-left border-separate border-spacing-0">
                            ...
                            <tbody className="divide-y divide-white/5 border-spacing-0">
                                {[...data.history].reverse().map((t, idx) => {
                                    ...
                                    const opPrice = parseFloat(t.price) || 1;
                                    const isBuy = t.side === 'BUY';
                                    const qty = t.qty || (parseFloat(t.amount_usd) / opPrice);
                                    const uPct = (((livePrice - opPrice) / opPrice) * 100).toFixed(1);
                                    const uUsd = (qty * (livePrice - opPrice)).toFixed(2);
                                    return (
                                        <tr key={idx} className="hover:bg-white/[0.02] text-left">
                                            <td className="px-6 py-4 font-mono text-[9px] text-zinc-500 whitespace-nowrap">{t.timestamp ? t.timestamp.split(' ')[1] : 'N/A'}</td>
                                            ...
                                            <td className="px-6 py-4 font-mono text-right text-zinc-400 text-[10px]">${opPrice < 1 ? opPrice.toFixed(4) : opPrice.toLocaleString(undefined, {maximumFractionDigits:0})}</td>
                                            <td className="px-6 py-4 font-mono text-right text-blue-400 text-[10px]">${livePrice < 1 ? livePrice.toFixed(4) : livePrice.toLocaleString(undefined, {maximumFractionDigits:0})}</td>
                                            <td className="px-6 py-4 font-mono text-right">
                                                {isBuy ? (
                                                    <div className="flex flex-col items-end leading-none">
                                                        <span className={parseFloat(uPct) >= 0 ? 'text-emerald-400' : 'text-rose-500'}>{parseFloat(uPct) >= 0 ? '+' : ''}{uPct}%</span>
                                                        <span className="text-[8px] text-zinc-600 font-bold mt-1">${uUsd}</span>
                                                    </div>
                                                ) : <span className="text-[9px] text-zinc-700 font-bold uppercase">Settled</span>}
                                            </td>
                                        </tr>
                                    );
                                })}
        ...
        {selectedToken && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 backdrop-blur-xl bg-black/60 overflow-y-auto outline-none">
                <div className="bg-[#18181b] border border-white/10 rounded-[3rem] w-full max-w-6xl shadow-2xl relative flex flex-col max-h-[95vh] overflow-hidden text-left">
                    <div className="p-8 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                        <div className="flex items-center gap-4 text-left">
                            ...
                            <div><h2 className="text-3xl font-black italic tracking-tighter uppercase m-0 leading-none">{selectedToken.id} / USD</h2><div className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-2 border border-white/10 px-2 py-1 rounded inline-block">Neural_Intelligence_Trace</div></div>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 ring-0 focus:ring-0 outline-none">
                        <div className="lg:col-span-8 space-y-6 text-left">
                            <div id="tradingview_chart" className="bg-black/40 border border-white/5 rounded-[2rem] h-[500px] w-full overflow-hidden flex flex-col items-center justify-center text-zinc-700 font-black uppercase text-[10px] tracking-widest">Initializing_Feed...</div>
                            <div className="bg-white/[0.02] border border-white/5 p-8 rounded-[2.5rem] shadow-inner text-left">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                    <div className="space-y-6">
                                        <div className="group/edu"><div className="text-[10px] text-zinc-100 font-bold flex items-center gap-2 mb-2 uppercase tracking-wider"><TrendingUp className="w-3.5 h-3.5 text-emerald-500" /> RSI (Strength)</div><p className="text-[11px] text-zinc-500 italic leading-relaxed m-0 text-left">Monitoring Velocity Exhaustion. We VETO buys above 70 to avoid buying the top, prioritizing entries below 40 where statistically oversold.</p></div>
                                        <div className="group/edu pt-4 border-t border-white/5"><div className="text-[10px] text-zinc-100 font-bold flex items-center gap-2 mb-2 uppercase tracking-wider"><Activity className="w-3.5 h-3.5 text-blue-500" /> MACD (Trend)</div><p className="text-[11px] text-zinc-500 italic leading-relaxed m-0 text-left">The Engine Room. We wait for a "Golden Cross" (Blue line crossing Orange) to confirm trend shift from distribution to accumulation.</p></div>
                                    </div>
                                    <div className="space-y-6">
                                    ...
                                    <div className="bg-blue-500/[0.03] border border-blue-500/10 rounded-3xl p-6 flex flex-col justify-between text-left">
                                        ...
                                        <div className="pt-4 border-t border-blue-500/10 font-bold"><div className="flex justify-between items-center text-[10px] uppercase font-black"><span className="text-zinc-600">Net Liquidity</span><span className="text-zinc-200">${(selectedToken.liq || 0).toLocaleString()}</span></div></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="lg:col-span-4 bg-white/[0.03] rounded-[2.5rem] p-8 border border-white/10 flex flex-col justify-between h-full text-left space-y-8">
                            <div className="space-y-8">
                                ...
                                <div className="mt-8 pt-6 border-t border-white/5 space-y-4">
                                    ...
                                    <div className="p-4 bg-emerald-500/[0.03] border border-emerald-500/10 rounded-2xl"><div className="flex items-center gap-2 mb-1"><ShieldCheck className="w-3 h-3 text-emerald-500" /><span className="text-[9px] text-emerald-500 font-black uppercase">Safety_v1.3</span></div><p className="text-[10px] text-zinc-500 italic mt-1 leading-tight">Protecting cash floor of $500. All trades require 2/3 consensus minimum.</p></div>
                                </div>
                            </div>
                            <a href={`https://dexscreener.com/${selectedToken.chain || 'base'}/${selectedToken.addr || ''}`} target="_blank" className="w-full py-5 bg-blue-600 hover:bg-blue-500 rounded-[2rem] text-center font-black italic text-sm tracking-tighter text-white no-underline mt-10 block uppercase transition-all shadow-lg active:scale-95">Explore Raw Pool Feed</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <footer className="pt-20 pb-12 opacity-30 text-left">
